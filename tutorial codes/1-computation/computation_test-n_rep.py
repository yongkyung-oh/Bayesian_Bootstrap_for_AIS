...
for nc, nr in grid_set:
    for n_rep in n_rep_set:
        ...
        X = np.round(X/np.sum(X)*1000).astype(int)
        ...
        result.append([(nc,nr), len(unique_routes), n_rep, get_weight_from_path(X, best_route['path']), computation_time])
        print('Grid_size:{} | n_routes: {} | N_rep:{} | Computation_time:{}'.format(nc*nr, len(unique_routes), n_rep, computation_time))

with open('computation_test-n_rep', 'wb') as f:
